...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CIlluminumAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CIlluminumAddress(rec->address.toStdString()) == CIlluminumAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CIlluminumAddress(value.toString().toStdString()).Get()))
            {
                editStatus = DUPLILMATE_ADDRESS;
                return false;
            }
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CIlluminumAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CIlluminumAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CIlluminumAddress(strAddress).Get()))
                {
                    editStatus = DUPLILMATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CIlluminumAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CIlluminumAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CIlluminumAddress address_parsed(sAddr);
