...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Illuminum"))
        return false;
...
{
    // Convert Illuminum:// to Illuminum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Illuminum://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Illuminum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Illuminum.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Illuminum.lnk
...
{
    return GetAutostartDir() / "Illuminum.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Illuminum.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Illuminum\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Illuminum app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Illuminum app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Illuminum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Illuminum-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Illuminum-Qt"));
