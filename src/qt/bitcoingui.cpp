...
{
    resize(900, 520);
    setWindowTitle(tr("Illuminum") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Illuminum");
    setStyleSheet("#Illuminum { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
    ...
    if (!fUseBlackTheme)
    {
        ...
        {
            progressBar->setStyleSheet("QProgressBar { color: #ffffff;background-color: #e8e8e8; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #0077d3 , stop: 1 #004377); border-radius: 7px; margin: 0px; }");
        }
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Illuminum address"));
    ...
    aboutAction = new QAction(QIcon(":icons/illuminum"), tr("&About Illuminum"), this);
    aboutAction->setToolTip(tr("Show information about Illuminum"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Illuminum"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Illuminum client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Illuminum client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setPixmap(QIcon(icon).pixmap(STATUSBAR_ILMONSIZE,STATUSBAR_ILMONSIZE));
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Illuminum network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    if(secs < 90*60)
    {
        ...
        labelBlocksIcon->setPixmap(QIcon(fUseBlackTheme ? ":/icons/black/synced" : ":/icons/synced").pixmap(STATUSBAR_ILMONSIZE, STATUSBAR_ILMONSIZE));
...
void BitcoinGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    ...
    if (style & CClientUIInterface::ILMON_ERROR) {
        nMBoxIcon = QMessageBox::Critical;
        nNotifyIcon = Notificator::Critical;
    }
    else if (style & CClientUIInterface::ILMON_WARNING) {
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Illuminum address or malformed URI parameters."));
    }
...
void BitcoinGUI::setEncryptionStatus(int status)
{
    ...
    {
    labelEncryptionIcon->setPixmap(QIcon(fUseBlackTheme ? ":/icons/black/lock_open" : ":/icons/lock_open").pixmap(STATUSBAR_ILMONSIZE,STATUSBAR_ILMONSIZE));
    ...
    case WalletModel::Locked:
        labelEncryptionIcon->setPixmap(QIcon(fUseBlackTheme ? ":/icons/black/lock_closed" : ":/icons/lock_closed").pixmap(STATUSBAR_ILMONSIZE,STATUSBAR_ILMONSIZE));
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if (nLastCoinStakeSearchInterval && nWeight)
    {
		...
		// 	nEstimateTime = TARGET_SPACING2 * nNetworkWeight / nWeight;
		// else
    ...
    nEstimateTime = TARGET_SPACING * nNetworkWeight / nWeight;
        ...
        labelStakingIcon->setPixmap(QIcon(fUseBlackTheme ? ":/icons/black/staking_on" : ":/icons/staking_on").pixmap(STATUSBAR_ILMONSIZE,STATUSBAR_ILMONSIZE));
        labelStakingIcon->setToolTip(tr("Staking.<br>Your weight is %1<br>Network weight is %2<br>Expected time to earn reward is %3").arg(nWeight).arg(nNetworkWeight).arg(text));
    }
    else
    {
        labelStakingIcon->setPixmap(QIcon(fUseBlackTheme ? ":/icons/black/staking_off" : ":/icons/staking_off").pixmap(STATUSBAR_ILMONSIZE,STATUSBAR_ILMONSIZE));
