...
void DarksendPage::toggleDarksend()
{
    ...
    else 
    {
        ...
        if (nAnonymizeIlluminumAmount == 0)
        {
...
void DarksendPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeIlluminumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIlluminumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeIlluminumAmount = strAnonymizeIlluminumAmount.remove(strAnonymizeIlluminumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIlluminumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeIlluminumAmount*COIN) nMaxToAnonymize = nAnonymizeIlluminumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeIlluminumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeIlluminumAmount));
        //strAnonymizeIlluminumAmount = strAnonymizeIlluminumAmount.remove(strAnonymizeIlluminumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIlluminumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
    ...
    QString strDenomBalance = QString().setNum(denomBalance, 'g', 4) + " ILM";
