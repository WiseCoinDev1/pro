...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIlluminumAmount"))
        settings.setValue("nAnonymizeIlluminumAmount", 1000);
    nAnonymizeIlluminumAmount = settings.value("nAnonymizeIlluminumAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIlluminumAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeIlluminumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIlluminumAmount:
            return QVariant(nAnonymizeIlluminumAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeIlluminumAmount:
            nAnonymizeIlluminumAmount = value.toInt();
            settings.setValue("nAnonymizeIlluminumAmount", nAnonymizeIlluminumAmount);
            emit AnonymizeIlluminumAmountChanged(nAnonymizeIlluminumAmount);
            break;
        default:
            break;
        }
