...
int nAnonymizeIlluminumAmount = 1000;
...
{
    if (mode & CRYPTO_LOCK) {
        ENTER_CRITILMAL_SECTION(*ppmutexOpenSSL[i]);
    } else {
        LEAVE_CRITILMAL_SECTION(*ppmutexOpenSSL[i]);
    }
}
...
#else
    const char* pszModule = "Illuminum";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Illuminum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Illuminum
    // Mac: ~/Library/Application Support/Illuminum
    // Unix: ~/.Illuminum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Illuminum";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Illuminum.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Illuminumd.pid"));
