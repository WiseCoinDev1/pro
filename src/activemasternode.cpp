...
//
// Bootup the masternode, look for a 500 ILM input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        LogPrintf("CActiveMasternode::ManageStatus() - Checking inbound connection to '%s'\n", service.ToString().c_str());
            ...
            if(!ConnectNode((CAddress)service, service.ToString().c_str())){
                notCapableReason = "Perhaps using port=<coin-port> is needed, Could not connect to " + service.ToString();
                status = MASTERNODE_NOT_CAPABLE;
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
                return;
            }
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Illuminum.conf */
...
{
    CIlluminumAddress address(collateralAddress);
