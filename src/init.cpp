...
void Shutdown()
{
    ...
    RenameThread("Illuminum-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Illuminum.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Illuminumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeilluminumamount=<n> " + _("Keep N Illuminum anonymized (default: 0)") + "\n";
...
bool AppInit2(boost::thread_group& threadGroup)
{
    ...
    _CrtSetReportFile(_CRT_WARN, CreateFileA("NUL", GENERILM_WRITE, 0, NULL, OPEN_EXISTING, 0, 0));
#endif
#if _MSC_VER >= 1400
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Illuminum is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Illuminum is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Illuminum version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Illuminum server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITILMAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Illuminum") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Illuminum to complete") << "\n";
    ...
    nAnonymizeIlluminumAmount = GetArg("-anonymizeilluminumamount", 0);
    if(nAnonymizeIlluminumAmount > 999999) nAnonymizeIlluminumAmount = 999999;
    if(nAnonymizeIlluminumAmount < 2) nAnonymizeIlluminumAmount = 2;
    ...
    LogPrintf("Anonymize Illuminum Amount %d\n", nAnonymizeIlluminumAmount);
