...
BOOL Win32RandomAccessFile::_Init( LPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
        _hFile = ::CreateFileW(path,GENERILM_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,
...
Status Win32Env::GetFileSize( const std::string& fname, uint64_t* file_size )
{
    ...
    HANDLE file = ::CreateFileW(wpath.c_str(),
        GENERILM_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
    LARGE_INTEGER li;
